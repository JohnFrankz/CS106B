 ...
 *
 * The implementation is compllicated by the fact that both the expression
 *
 *      RAND_MAX + 1
 *
 ...
 * --------------------------------
 * The code for randomReal is similar to that for randomInteger without the 
 ...
 * ----------------------------------
 * The code for randomChance calls randomReal(0, 1) and then checks whether the
 * result is less than the requested probability.
 */
 ...
 * -----------------------------------
 * The setRandomSeed function simply forwards its argument to srand.  The call
 * initRandomSeed is required to set the initialized flag.
 */
 ...
 * ------------------------------------
 * The initRandomSeed function declares a static variable that keeps track of 
 * whether the seed has been initialized.  The first time initRandomSeed is 
 * called, initialized is false, so the seed is set to the current time.
 */
void initRandomSeed() {
